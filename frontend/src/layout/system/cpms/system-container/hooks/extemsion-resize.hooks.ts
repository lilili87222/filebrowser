...
export default function () {
  onMounted(() => {
    const drawerEle = document.querySelector('.el-drawer') as HTMLElement
    // 创建触发拖拽的元素
    const dragItem = document.createElement('div')
    // 将元素放置到抽屉的左边边缘
    dragItem.style.cssText =
      'height: 100%;width: 5px;cursor: w-resize;position: absolute;left: 0;'
    ...
    dragItem.onmousedown = () => {
      ...
      document.onmousemove = function (moveEvent) {
        ...
        const width30 = document.body.clientWidth * 0.3
